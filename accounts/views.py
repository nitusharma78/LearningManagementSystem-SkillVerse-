...
from django.contrib import messages
from accounts.models import User  # अपने मॉडल को import करें
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    # User instance बनाओ लेकिन अभी save मत करो
                    user = form.save(commit=False)
                    user.is_student = True  # student flag सेट करो
                    user.save()
                    print(user)
                    # Student प्रोफ़ाइल भी बनाओ
...
def login_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('home')  # या 'student_dashboard'
            else:
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    messages.success(
                        request,
                        f"Account for lecturer {lecturer.get_full_name} has been created. "
                        f"An email with account credentials will be sent to {lecturer.email}.",
                    )
                    return redirect("lecturer_list")
            except Exception as e:
                messages.error(request, f"Error creating lecturer account: {str(e)}")
        else:
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    messages.success(
                        request,
                        f"Account for {user.get_full_name} has been created. "
                        f"An email with account credentials will be sent to {user.email}."
                    )
